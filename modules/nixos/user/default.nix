...
in
{
  options.user = with types; {
    ...
    passwordlessSudo = mkOpt bool false "Whether to allow passwordless sudo for the user.";
    trustedUser = mkOpt bool false "Whether the user should be added to the trusted-users list of nix";
  };

  config = {
    users.mutableUsers = false;
    nix.settings.trusted-users = mkIf cfg.trustedUser [
      "${cfg.name}"
    ];
